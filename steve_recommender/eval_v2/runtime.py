...
def _module_is_local(module_name: str, *, expected_root: Path) -> bool:
    ...
    if not module_file:
        return False
    return Path(module_file).resolve().is_relative_to(expected_root)


def _ensure_local_steve_imports() -> None:
    for path in (str(LOCAL_STEVE_RL_PATH), str(LOCAL_STEVE_PATH)):
    ...
    if not Path(eve.__file__).resolve().is_relative_to(LOCAL_STEVE_PATH):
        raise ImportError(f"Failed to import local stEVE package from {LOCAL_STEVE_PATH}")
    if not Path(eve_rl.__file__).resolve().is_relative_to(LOCAL_STEVE_RL_PATH):
        raise ImportError(
