...
def _parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Sample the anatomy input set for E2")
    parser.add_argument("--pool-path", type=Path, default=DEFAULT_POOL_PATH)
    parser.add_argument(
        "--output",
        type=Path,
        default=PROJECT_ROOT / "results" / "experimental_prep" / "sample_1000_e2.json",
    )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    print(f"[sample_e2_anatomies] wrote {len(payload['selected_anatomies'])} anatomies to {args.output}")
    return 0
